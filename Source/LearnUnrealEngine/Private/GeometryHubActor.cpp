...
void AGeometryHubActor::DoActorSpawn1()
{
	...
	if (World)
	{
		...
		{
			FTransform GeometryTransform = FTransform(FRotator::ZeroRotator, FVector(0, 300.0f * i, 300.0f));
...
void AGeometryHubActor::DoActorSpawn2()
{
	...
	if (World)
	{
		for (int32 i = 0; i < 10; i++)
		{
			FTransform GeometryTransform = FTransform(FRotator::ZeroRotator, FVector(0, 300.0f * i, 700.0f));
